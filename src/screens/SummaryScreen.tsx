  ...
  return (
    <div style={{ color: '#444', lineHeight: 1.6, direction: 'rtl', textAlign: 'right', wordBreak: 'break-word', overflowWrap: 'anywhere' }}>
      {markdown.split('\n').filter(Boolean).map((line, li) => (
        <p key={li} className="mb-2">{line.replace(/\*\*/g, '')}</p>
      ))}
    </div>
  );
}
    ...
    <div style={{ direction: 'rtl', borderRadius: 14, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.08)', background: '#fff', border: '1.5px solid #ccc' }}>
      <button
        style={{
          ...
          fontSize: '1rem', color: '#1a1a2e', fontFamily: 'var(--font-heading)',
      ...
      <div style={{ display: 'grid', gridTemplateRows: acc.open ? '1fr' : '0fr', transition: 'grid-template-rows 0.3s ease', overflow: 'hidden' }}>
        <div style={{ overflow: 'hidden' }}>
          <div style={{ color: '#444', lineHeight: 1.6, padding: isMobile ? '0 12px 10px' : '0 18px 14px', textAlign: 'right', wordBreak: 'break-word', overflowWrap: 'anywhere' }}>
            {acc.body.split('\n').filter(Boolean).map((line, li) => <p key={li} className="mb-2">{line}</p>)}
          </div>
        </div>
      </div>
    </div>
...
export default function SummaryScreen({ agentResponse, firstName, runId, isMuted, onMuteToggle }: Props) {
  ...
  const gridCls = 'grid gap-3 ' + (isMobile ? '' : 'grid-cols-2');
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        padding: isMobile ? '0 44px' : '0 56px',
        overflow: 'hidden',
      }}>
        ...
        {/* Analysis text */}
        <p className="leading-relaxed" style={{ direction: 'rtl', textAlign: 'right', color: '#333', fontSize: isMobile ? '0.95rem' : '1.05rem', marginBottom: isMobile ? 10 : 24, wordBreak: 'break-word', overflowWrap: 'anywhere' }}>
          {agentResponse.enginesText}
        </p>

        {/* Programs title */}
        <h2 className="font-bold" style={{ direction: 'rtl', textAlign: 'right', fontFamily: 'var(--font-heading)', color: '#1a1a2e', fontSize: isMobile ? '0.95rem' : '1.05rem', marginBottom: isMobile ? 8 : 16 }}>
          ...
          <div className={gridCls} style={{ direction: 'rtl' }}>
            {firstBatch.map((acc, i) => (
              <AccordionItem
                ...
                isMobile={isMobile}
                onToggle={() => {
                  if (!acc.open) trackProgramClick(acc.title, i);
                  toggleAccordion(i);
                }}
              />
            ))}
              ...
              <div className={gridCls + ' mt-3'} style={{ direction: 'rtl' }}>
                {extraAccordions.map((acc, i) => (
                  <AccordionItem
                    ...
                    isMobile={isMobile}
                    onToggle={() => {
                      if (!acc.open) trackProgramClick(acc.title, accordions.length + i);
                      toggleExtra(i);
                    }}
                  />
                ))}
        ...
        <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ display: 'flex', gap: isMobile ? 8 : 10, flexWrap: 'nowrap', justifyContent: 'center' }}>
